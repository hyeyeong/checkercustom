...
public class EpubSVGCheck implements DocumentValidator
{
  ...
  void checkViewBox(String svgDocEntry, Document doc)
  {
    ...
    for (int i = 0; i < n.getLength(); i++)
    {
      ...
      String lang = svgElement.getAttribute("xml:lang");
      
      if (lang == null || lang.length() == 0)
      {
        report.message(MessageId.KEAT_037, EPUBLocation.create(svgDocEntry, XmlDocParser.getElementLineNumber(svgElement), XmlDocParser.getElementColumnNumber(svgElement)));
      }
      
      NodeList cn = n.item(i).getChildNodes();
      ...
      if (!hasTitle || !hasDesc)
      {
    	  report.message(MessageId.KEAT_038, EPUBLocation.create(svgDocEntry, XmlDocParser.getElementLineNumber(svgElement), XmlDocParser.getElementColumnNumber(svgElement)));
      }
