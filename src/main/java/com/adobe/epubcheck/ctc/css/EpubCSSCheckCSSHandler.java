...
public class EpubCSSCheckCSSHandler implements CssContentHandler, CssErrorHandler
{
  ...
  public void declaration(CssGrammar.CssDeclaration declaration)
  {
    if (declaration.getName() != Optional.<String>absent())
    {
      ...
      if ("overflow".equals(text))
      {
    	  for (CssGrammar.CssConstruct cssc : declaration.getComponents())
    	  {
    		  if ("hidden".equals(cssc.toCssString()))
    		  {
    			  report.message(MessageId.KEAT_015, EPUBLocation.create(this.getPath(), cssc.getLocation().getLine(), cssc.getLocation().getColumn()), text);
    		  }
    	  }
      }
      
      if ("width".equals(text))
      {
    	  for (CssGrammar.CssConstruct cssc : declaration.getComponents())
    	  {
    		  if ("0px".equals(cssc.toCssString()))
    		  {
    			  report.message(MessageId.KEAT_016, EPUBLocation.create(this.getPath(), cssc.getLocation().getLine(), cssc.getLocation().getColumn()), text);
    		  }
    	  }
      }
      /*
      if ("backgroud-image".equals(text))
      {
    	  for (CssGrammar.CssConstruct cssc : declaration.getComponents())
    	  {
    		  if (StringUtil.isEmpty(cssc.toCssString()) || "url('')".trim().equals(cssc.toCssString()) || "url(\"\")".trim().equals(cssc.toCssString()))
    		  {
    			  report.message(MessageId.KEAT_017, EPUBLocation.create(this.getPath(), cssc.getLocation().getLine(), cssc.getLocation().getColumn()), text);
    		  }
    	  }
      }
      */
      
      if ("text-align".equals(text))
      {
    	  for (CssGrammar.CssConstruct cssc : declaration.getComponents())
    	  {
    		  if ("justify".equals(cssc.toCssString()))
    		  {
    			  report.message(MessageId.KEAT_019, EPUBLocation.create(this.getPath(), cssc.getLocation().getLine(), cssc.getLocation().getColumn()), text);
    		  }
    	  }
      }
      ...
      {
    	  report.message(MessageId.KEAT_013, EPUBLocation.create(this.getPath(), declaration.getLocation().getLine(), declaration.getLocation().getColumn()), text);
      }
      
      if ("font-size".equals(text))
      {
    	  for (CssGrammar.CssConstruct cssc : declaration.getComponents())
    	  {
    		  if (cssc.toCssString().toLowerCase().contains("pt") || cssc.toCssString().toLowerCase().contains("px") || cssc.toCssString().toLowerCase().contains("x-small"))
    		  {
    			  report.message(MessageId.KEAT_018, EPUBLocation.create(this.getPath(), cssc.getLocation().getLine(), cssc.getLocation().getColumn()), text);
    		  }
    	  }
      }
  ...
  {
	  String declarationName = declaration.getName().get();
    ...
    */
    //if (declarationName.trim().startsWith("backgroud-") || declarationName.trim().equals("backgroud-image"))
	if (declarationName.equals("background-image"))
    {
    	if (!isGlobalFixedFormat)
    	{
    		report.message(MessageId.KEAT_012, getCorrectedEPUBLocation(path, declaration.getLocation().getLine(), declaration.getLocation().getColumn(), declaration.toCssString()), declarationName);
    	}
    	
    	for (CssGrammar.CssConstruct cssc : declaration.getComponents())
  	  	{
  		  if (cssc.toCssString() == null || StringUtil.isEmpty(cssc.toCssString()) || cssc.toCssString().equals("") || cssc.toCssString().equals("url('')") || cssc.toCssString().equals("url(\"\")") || cssc.toCssString().equals("none")) 
  		  {
  			report.message(MessageId.KEAT_017, getCorrectedEPUBLocation(path, declaration.getLocation().getLine(), declaration.getLocation().getColumn(), declaration.toCssString()), declarationName);
    	...
    	if (isGlobalFixedFormat)
    	{
	    	for (CssGrammar.CssConstruct cssc : declaration.getComponents())
	    		...
	    		if (cssc.toCssString() == null || StringUtil.isEmpty(cssc.toCssString()) || cssc.toCssString().equals("") || cssc.toCssString().equals("url('')") || cssc.toCssString().equals("url(\"\")") || cssc.toCssString().equals("none")) 
	  		  {
	  			report.message(MessageId.KEAT_020, getCorrectedEPUBLocation(path, declaration.getLocation().getLine(), declaration.getLocation().getColumn(), declaration.toCssString()), declarationName);
	  			//report.message(MessageId.KEAT_017, getCorrectedEPUBLocation(path, declaration.getLocation().getLine(), declaration.getLocation().getColumn(), declaration.toCssString()), declarationName);
