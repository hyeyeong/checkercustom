...
{
  @Test
  public void EpubTest() throws Exception
  {
      String inputPath = "D:/Works/eBook/Source/workspace/epubcheck-master/target/fix_bg2.epub";
	  File epub = new File(inputPath);
	  /*
	  CheckingReport report = new CheckingReport(inputPath);
	  report.setReportingLevel(4);
	  	*/  
	  EpubCheck check = new EpubCheck(epub);
	  check.doValidate();
	  /*
	  CheckerMetadata checkMeta = report.getChecker();
	  PublicationMetadata publication = report.getPublication();
	  List<CheckMessage> messages = report.getMessages();
	  
	  CheckMessage message = messages.get(0);
	  */
  ...
  {
	  String inputPath = "D:/Works/eBook/Source/workspace/epubcheck-master/target/invalid-ncx.epub";
	  File epub = new File(inputPath);
    URL expectedUrl = common.class.getResource("api");
    String outputPath = expectedUrl.getPath();
    File actualResults = new File(outputPath + "/PrintWriter_Actual.json");
    File expectedResults = new File(outputPath + "/PrintWriter_Expected.json");
